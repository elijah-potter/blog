  ...
  useEffect(() => {
    ...
    const fetchData = async (): Promise<BarDatum[]> => {
      ...
      return rels.map((r, i) => {
        ...
        const end   = i < rels.length - 1 ? new Date(rels[i + 1].published_at) : new Date();
        ...
        return { tag: r.tag_name, start, end, rate: +(total / days).toFixed(2) };
      });
    };
    ...
    const draw = (data: BarDatum[]) => {
      ...
      const x = d3.scaleTime()
        .domain([d3.min(data, (d: BarDatum) => d.start),
                 d3.max(data, (d: BarDatum) => d.end)])
        .range([margin.left, width - margin.right]);

      const y = d3.scaleLinear()
        .domain([0, d3.max(data, (d: BarDatum) => d.rate)]).nice()
    ...
    <>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="mt-6 text-gray-700 dark:text-gray-300 space-y-2">
          ...
          <p>
            This visualization shows <strong>GitHub release downloads/day</strong> for each version, 
            based on real data from GitHub's API. Bar <strong>height</strong> represents daily download velocity, 
            while bar <strong>width</strong> spans how long each release was the latest.
          </p>
          <p>
            Use it to quickly gauge adoption, identify impactful releases, and better understand your project's momentum over time.
            Hover over each bar to see exact download rates and date ranges.
          </p>
        </div>
      </div>
    </>
