...
import { and, desc, eq, isNotNull, isNotNull, isNull } from "drizzle-orm";
...
export async function getCommentsForPost(postSlug: string): Promise<Comment[]> {
	return await db
		.select()
		.from(commentsTable)
		.where(and(eq(commentsTable.post, postSlug), isNull(commentsTable.deleted)));
}
...
export async function getCommentFromId(
	...
	return (
		await db.select().from(commentsTable).where(and(eq(commentsTable.id, id), isNull(commentsTable.deleted)))
	)[0];
}

/** Performs a soft delete by setting the delete timestamp to now. */
export async function deleteComment(id: number): Promise<void> {
  await db.update(commentsTable).set({ deleted: new Date()}).where(and(eq(commentsTable.id, id), isNull(commentsTable.deleted)));
}
