...
The bit that I find most interesting here is that there is native support in macOS for the exact speech-to-text model that Talkify uses.
...
The macOS set up is difficult to find and it's frankly ugly.
It lacks visual flair.
Talkify fixes this.
It takes that same great model and wraps it it a prettier UI that makes the whole experience more satisfying.
...
On the other hand, if the Harper offering begins expanding to be more than just grammar and checking, and instead towards a fuller suite of tools, we could totally include high-quality speech-to-text as part of that.
